...
# Function that performs the parallel computation
computeShareReject <- function(i) {
  MTests(df$N[i], df$M[i], "tweedie", 0.05)
}

# Use the foreach package for parallel computation
df$share_reject <- foreach(i = 1:nrow(df), .packages = c("tweedie", "stats"), .combine = c) %dopar% {
  computeShareReject(i)
}
